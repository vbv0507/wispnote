...
export const uploadAttachment = async (req, res, next) => {
  try {
    ...
    const MAX_STORAGE_BYTES = 10 * 1024; // 10 KB
    ...
    if (newTotalSize > MAX_STORAGE_BYTES) {
      return res.status(400).json({ 
        error: `Storage limit exceeded. This chat room can hold up to 10KB of attachments. Please upload larger files to Google Drive or OneDrive and share the link in the chat.` 
      });
    }
